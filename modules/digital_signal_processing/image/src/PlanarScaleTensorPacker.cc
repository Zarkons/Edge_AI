...
namespace dsp
{
    namespace image
    {
        ...
        PlanarScaleTensorPacker::PlanarScaleTensorPacker(bool swap_rb)
            : m_swap_rb(swap_rb) {}
                                           ...
                                           size_t tensor_size)
        {
            ...
            // Loop Unswitching Optimization: Branch first, then execute uninterrupted sequential loops
            if (m_swap_rb)
            {
                // CASE 1: Source data is BGR -> Flip to RGB on scatter
            ...
            {
                // CASE 2: Source data is already RGB -> Map directly without channel swapping
                for (size_t i = 0; i < plane_stride; ++i)
                {
                    size_t hwc_base = i * channels;

                    uint8_t r_val = preprocessed_img[hwc_base + 0];
                    uint8_t g_val = preprocessed_img[hwc_base + 1];
                    uint8_t b_val = preprocessed_img[hwc_base + 2];
