...
namespace
{
    ...
    std::vector<std::string> getLabelNames()
    {
        ...
        const std::string label_names_file = (workspace_dir != nullptr)
                                                 ? (fs::path(workspace_dir) / "demo_applications/simple_audio_recognition/train/build_dir/label_names.txt").string()
                                                 : "demo_applications/simple_audio_recognition/train/build_dir/label_names.txt";
